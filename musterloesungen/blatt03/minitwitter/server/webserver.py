...
class Response:
    ...
    def send_template(self, template, dictionary={}, code=None, headers=None):
        ...
        # add some general fields to dictionary
        dictionary.update({'request': self.server.request}) # the request object
        if hasattr(self.server.request, 'session'):  # add the session dictionary if present
            dictionary.update({'session': self.server.request.session})
