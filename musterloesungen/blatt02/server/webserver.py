...
class Response:
    ...
    def commit(self):
        ...
        if self.body and 'Content-Length' not in self.headers:
            if isinstance(self.body, str):  # we need length in bytes, so we might have to convert it first
...
class Webserver:
    ...
    def set_templating(self, templating):
        if templating in self.templating_available:
            ...
            raise Exception("Invalid templating engine name: {}. Available are: {}.".format(templating,
                                                                                            self.templating_available))
